import { createContext, useContext, useState, useRef, useEffect, useCallback, createElement, Fragment, Component } from 'react';
import { ResizeSensor } from 'css-element-queries';
import { string, oneOfType, object, array, oneOf, number, bool, func } from 'prop-types';
...
(function (ResizeType) {
    ...
    ResizeType["NW"] = "resize-nw";
    ResizeType["NE"] = "resize-ne";
    ResizeType["SW"] = "resize-sw";
    ResizeType["SE"] = "resize-se";
...
function createResize(store) {
    function onResize(space, targetSize, resizeType, startSize, originalX, originalY, x, y, minimumAdjust, maximumAdjust) {
        var adjustment = startSize +
    ...
    return {
        startResize: function (resizeType, e, space, targetSize, endEvent, moveEvent, getCoords) {
            ...
            var minimumAdjust = coalesce(space.minimumSize, 20) - size + targetSize.resized;
            var maximumAdjust = space.maximumSize ? space.maximumSize - size + targetSize.resized : undefined;
            var lastX = 0;
            var lastY = 0;
            var moved = false;
            var resize = function (x, y) {
                return onResize(space, targetSize, resizeType, startSize, originalCoords.x, originalCoords.y, x, y, minimumAdjust, maximumAdjust);
            };
            ...
            var removeListener = function () {
                ...
                if (space.onResizeEnd) {
                    ...
                    space.onResizeEnd(Math.floor(resizeType === ResizeType.Left || resizeType === ResizeType.Right ? currentRect.width : currentRect.height), currentRect);
                }
            };
...
function createStore() {
        ...
        var addDefaultOrders = function (spaces) {
            ...
            anchorTypes.forEach(function (t) {
                ...
                zIndices.forEach(function (i) {
                    ...
                    result = __spreadArrays(result, __spreadArrays(orderedSpaces, unorderedSpaces.map(function (c, idx) { return (__assign(__assign({}, c), { order: maxOrder + idx + 1 })); })));
                });
            });
            return __spreadArrays(result, spaces.filter(function (s) { return s.anchor === undefined; }));
        };
    ...
    store.startMouseResize = function (resizeType, space, size, event) {
        resize.startResize(resizeType, event, space, size, EndEvent.Mouse, MoveEvent.Mouse, function (e) { return ({
            x: e.clientX,
            y: e.clientY,
        }); });
    };
    store.startTouchResize = function (resizeType, space, size, event) {
        resize.startResize(resizeType, event, space, size, EndEvent.Touch, MoveEvent.Touch, function (e) { return ({
...
var ParentContext = createContext(undefined);
var DOMRectContext = createContext(undefined);
var LayerContext = createContext(undefined);
var OptionsContext = createContext({});
...
var commonProps = {
    ...
    as: string,
    centerContent: oneOf([CenterType.None, CenterType.Vertical, CenterType.HorizontalVertical]),
    zIndex: number,
    scrollable: bool,
    trackSize: bool,
    onClick: func,
...
function useSpace(props) {
    ...
    var layer = useContext(LayerContext);
    var debug = useContext(OptionsContext).debug;
    var spaceId = useState(props.id || "s" + shortuuid())[0];
    var elementRef = useRef();
    var resizeSensor = useRef();
    var _a = useState(), domRect = _a[0], setDomRect = _a[1];
    ...
    var resizeHandles = useSpaceResizeHandles(store, space, props.position);
    useEffect(function () {
...
function useSpaceResizeHandles(store, space, position) {
    var mouseHandles = [];
    if (position && position.rightResizable) {
        mouseHandles.push({
            ...
            onMouseDown: function (event) { return store.startMouseResize(ResizeType.Right, space, space.width, event); },
            onTouchStart: function (event) { return store.startTouchResize(ResizeType.Right, space, space.width, event); },
        });
    }
    if (position && position.leftResizable) {
        mouseHandles.push({
            ...
            onMouseDown: function (event) { return store.startMouseResize(ResizeType.Left, space, space.width, event); },
            onTouchStart: function (event) { return store.startTouchResize(ResizeType.Left, space, space.width, event); },
        });
    }
    if (position && position.topResizable) {
        mouseHandles.push({
            ...
            onMouseDown: function (event) { return store.startMouseResize(ResizeType.Top, space, space.height, event); },
            onTouchStart: function (event) { return store.startTouchResize(ResizeType.Top, space, space.height, event); },
        });
    }
    if (position && position.bottomResizable) {
        mouseHandles.push({
            ...
            onMouseDown: function (event) { return store.startMouseResize(ResizeType.Bottom, space, space.height, event); },
            onTouchStart: function (event) { return store.startTouchResize(ResizeType.Bottom, space, space.height, event); },
        });
    }
    return {
...
var Centered = function (props) { return createElement("div", { className: "spaces-centered" }, props.children); };

var CenteredVertically = function (props) { return createElement("div", { className: "spaces-centered-vertically" }, props.children); };

function applyCentering(children, centerType) {
    switch (centerType) {
        case CenterType.Vertical:
            return createElement(CenteredVertically, null, children);
        case CenterType.HorizontalVertical:
            return createElement(Centered, null, children);
    }
    return children;
}
var Space = /** @class */ (function (_super) {
    ...
    Space.prototype.render = function () {
        return createElement(SpaceInner, __assign({}, this.props, { wrapperInstance: this }));
    };
    return Space;
}(Component));
var SpaceInner = function (props) {
    ...
    var outerClasses = __spreadArrays(["spaces-space", space.children.find(function (s) { return s.resizing; }) ? "spaces-resizing" : undefined], userClasses.map(function (c) { return c + "-container"; })).filter(function (c) { return c; });
    var innerClasses = __spreadArrays(["spaces-space-inner"], userClasses);
    ...
    return (createElement(Fragment, null,
        resizeHandles.mouseHandles.map(function (r) { return (createElement("div", __assign({}, r))); }),
        createElement(props.as || "div", __assign({
            id: space.id,
            ref: elementRef,
            className: outerClasses.join(" "),
        }, events), createElement("div", { className: innerClasses.join(" "), style: innerStyle },
            createElement(ParentContext.Provider, { value: space.id },
                createElement(LayerContext.Provider, { value: undefined },
                    createElement(DOMRectContext.Provider, { value: domRect }, centeredContent)))))));
};

var anchoredProps = __assign(__assign({}, commonProps), {
    size: oneOfType([string, number]).isRequired,
    order: number,
    resizable: bool,
    handleSize: number,
    handlePlacement: oneOf([ResizeHandlePlacement.Inside, ResizeHandlePlacement.OverlayBoundary, ResizeHandlePlacement.OverlayInside]),
    minimumSize: number,
    maximumSize: number,
    onResizeStart: func,
    onResizeEnd: func,
...
var LeftResizable = function (_a) {
    ...
    return (createElement(Space, __assign({}, props, { type: Type.Anchored, anchor: AnchorType.Left, position: { left: 0, top: 0, bottom: 0, rightResizable: true, width: size } }), children));
};
...
var Left = function (_a) {
    ...
    return (createElement(Space, __assign({}, commonProps, { type: Type.Anchored, anchor: AnchorType.Left, position: { left: 0, top: 0, bottom: 0, rightResizable: resizable, width: size } }), children));
};
Left.propTypes = anchoredProps;
var TopResizable = function (_a) {
    var children = _a.children, size = _a.size, props = __rest(_a, ["children", "size"]);
    return (createElement(Space, __assign({}, props, { type: Type.Anchored, anchor: AnchorType.Top, position: { left: 0, top: 0, right: 0, bottomResizable: true, height: size } }), children));
};
TopResizable.propTypes = resizableProps;
var Top = function (_a) {
    var size = _a.size, children = _a.children, resizable = _a.resizable, commonProps = __rest(_a, ["size", "children", "resizable"]);
    return (createElement(Space, __assign({}, commonProps, { type: Type.Anchored, anchor: AnchorType.Top, position: { left: 0, top: 0, right: 0, bottomResizable: resizable, height: size } }), children));
};
Top.propTypes = anchoredProps;
var RightResizable = function (_a) {
    var children = _a.children, size = _a.size, props = __rest(_a, ["children", "size"]);
    return (createElement(Space, __assign({}, props, { type: Type.Anchored, anchor: AnchorType.Right, position: { bottom: 0, top: 0, right: 0, leftResizable: true, width: size } }), children));
};
RightResizable.propTypes = resizableProps;
var Right = function (_a) {
    var size = _a.size, children = _a.children, resizable = _a.resizable, commonProps = __rest(_a, ["size", "children", "resizable"]);
    return (createElement(Space, __assign({}, commonProps, { type: Type.Anchored, anchor: AnchorType.Right, position: { bottom: 0, top: 0, right: 0, leftResizable: resizable, width: size } }), children));
};
Right.propTypes = anchoredProps;
var BottomResizable = function (_a) {
    var children = _a.children, size = _a.size, props = __rest(_a, ["children", "size"]);
    return (createElement(Space, __assign({}, props, { type: Type.Anchored, anchor: AnchorType.Bottom, position: { bottom: 0, left: 0, right: 0, topResizable: true, height: size } }), children));
};
BottomResizable.propTypes = resizableProps;
var Bottom = function (_a) {
    var size = _a.size, children = _a.children, resizable = _a.resizable, commonProps = __rest(_a, ["size", "children", "resizable"]);
    return (createElement(Space, __assign({}, commonProps, { type: Type.Anchored, anchor: AnchorType.Bottom, position: { bottom: 0, left: 0, right: 0, topResizable: resizable, height: size } }), children));
};
Bottom.propTypes = anchoredProps;

var Custom = function (_a) {
    var children = _a.children, left = _a.left, top = _a.top, right = _a.right, bottom = _a.bottom, width = _a.width, height = _a.height, anchorSize = _a.anchorSize, anchor = _a.anchor, isPositioned = _a.isPositioned, resizable = _a.resizable, props = __rest(_a, ["children", "left", "top", "right", "bottom", "width", "height", "anchorSize", "anchor", "isPositioned", "resizable"]);
    var position;
    var type = Type.Positioned;
    if (!isPositioned) {
        if (anchor === AnchorType.Left) {
    ...
    return (createElement(Space, __assign({}, props, { type: type, anchor: anchor, position: position }), children));
};
Custom.propTypes = __assign(__assign({}, commonProps), {
    ...
    isPositioned: bool,
    anchor: oneOf([AnchorType.Left, AnchorType.Top, AnchorType.Right, AnchorType.Bottom]),
    anchorSize: oneOfType([string, number]),
    resizable: bool,
    handleSize: number,
    minimumSize: number,
    maximumSize: number,
...
var Fill = function (props) { return (createElement(Space, __assign({}, props, { type: Type.Fill, position: { left: 0, top: 0, right: 0, bottom: 0 } }), props.children)); };
...
var Fixed = function (_a) {
    ...
    return (createElement(Space, __assign({}, commonProps, { type: Type.Fixed, position: { width: width, height: height } }), children));
};
...
var Layer = function (props) { return createElement(LayerContext.Provider, { value: props.zIndex }, props.children); };
Layer.propTypes = {
...
var Positioned = function (_a) {
    ...
    return (createElement(Space, __assign({}, props, { type: Type.Positioned, position: { left: left, top: top, right: right, bottom: bottom, width: width, height: height } }), props.children));
};
...
var Info = function (props) {
    var domRect = useContext(DOMRectContext);
    if (domRect) {
...
var ViewPort = function (_a) {
    ...
    return (createElement(Space, __assign({}, commonProps, { type: Type.ViewPort, position: { left: left || 0, top: top || 0, right: right || 0, bottom: bottom || 0 } }), children));
};
...
var Options = function (_a) {
    ...
    return createElement(OptionsContext.Provider, { value: opts }, children);
};

export { AnchorType, Bottom, BottomResizable, CenterType, Centered, CenteredVertically, Custom, Fill, Fixed, Info, Layer, Left, LeftResizable, Options, Positioned, ResizeHandlePlacement, Right, RightResizable, Top, TopResizable, ViewPort, anchoredProps, resizableProps };
